...
export function showBanner() {
  ...
  const tagline =
    ...
    chalk.dim('  v1.0.0  ·  MIT License  ·  github.com/dawoodriy-hub/dawood-ai\n\n') +
    chalk.cyan('  Type ') +
...
export function showHelp() {
  const cmds = [
    ...
    ['/export',        'Export chat to a markdown file'],
    ['/exit',          'Quit Dawood AI'],
  ];

  const rows = cmds
    .map(([cmd, desc]) => chalk.cyan(cmd.padEnd(20)) + chalk.gray(desc))
    .join('\n');
...
export function showConfig(config, configPath) {
  const maskedKey = config.apiKey
    ? chalk.green('sk-••••••••' + config.apiKey.slice(-6))
